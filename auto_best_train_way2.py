...
if __name__ == '__main__':
    ...
    json_path = f'{all_save_path}/json/class_indices_{project_name}.json'
    ...
    valid_loader = torch.utils.data.DataLoader(validate_dataset,
                                               batch_size=batch_size, shuffle=False,
                                               num_workers=0)
    weights_save_path = f"{all_save_path}/weights/{project_name}_clean"
    os.makedirs(weights_save_path, exist_ok=True)
    log_path = f'{all_save_path}/log/{project_name}_cleantest_{epochs}epochs_clean.txt'
    train(len(class_indict), epochs, train_loader, valid_loader, val_dataset_num, test_loader, test_dataset_num,
          log_path,
          weights_save_path)
